...
async function executeEthereum(network, contractAddress, contractAbi, functionName, value, numberOfTransactions) {
    ...
    async function sendTransactions() {
        async function executeTransaction(val, gasPriceMultiplier, index, nonce) {
            try {
                ...
                const tx = await contract[functionName](val, { gasPrice, gasLimit, nonce });
        ...
        for (const multiplier of gasPriceMultipliers) {
            ...
            const sendAndMeasure = async () => {
                ...
                for (let i = 0; i < numberOfTransactions; i++) {
                    ...
                    const transactionPromise = executeTransaction(value, multiplier, i + 1,nonce); // Pass transaction index
