...
// const logFile = path.join(__dirname, 'logs/Ethereum_logs_individual.txt');
// const tpsAndLatencyLog = path.join(__dirname, 'logs/Eth_TPS&AvgLatency_log.txt');
// const ethWalletResource = path.join(__dirname, "logs/EthWalletResourceUsage.txt");
// const resourceUsageLog = path.join(__dirname, 'logs/Ethereum_Resource_Usage.txt');
async function BlockGeneration(provider) { // Function to measure block generation rate
...
function measureLatency(totalLatency, numOfTransactions) { // Measure average latency
...
async function executeEthereum(network, contractAddress, contractAbi, functionName, params, numberOfTransactions) {
    ...
    async function sendTransactions() {
        ...
        for (const multiplier of gasPriceMultipliers) {
            ...
            await Promise.all([sendAndMeasure(), BlockGeneration(provider)]);
